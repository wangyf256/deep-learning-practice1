...
CONV2_SIZE = 3                  # 第二层卷积层的尺寸


# 第三层卷积层的尺寸和深度
CONV3_DEEP = 64                 # 第三层卷积层的深度
CONV3_SIZE = 5                 # 第三层卷积层的尺寸

# 第四层卷积层的尺寸和深度
CONV4_DEEP = 32                 # 第四层卷积层的深度
CONV4_SIZE = 3                 # 第四层卷积层的尺寸


# 全连接层的节点数
FC_SIZE = 512

BATCH_NODE = 256                # batch的大小
...
MODEL_SAVE_PATH = "C:/Users/Raytine/Documents/deep learning/practice1/model"
...
def inference(input_tensor, regularizer):
    ...
    # 声明第四层卷积层的变量并实现前向传播过程，与第一层相似
    with tf.variable_scope('layer4-conv3'):
        conv3_weights = tf.get_variable("weight", [CONV3_SIZE, CONV3_SIZE, CONV2_DEEP, CONV3_DEEP], initializer=tf.truncated_normal_initializer(stddev=0.1))
    ...
    # 声明第五层卷积层的变量并实现前向传播过程，与第一层相似
    with tf.variable_scope('layer5-conv4'):
        conv4_weights = tf.get_variable("weight", [CONV4_SIZE, CONV4_SIZE, CONV3_DEEP, CONV4_DEEP], initializer=tf.truncated_normal_initializer(stddev=0.1))
...
def train(mnist):
    ...
    def calculate():
        with tf.Session() as sess:
            ...
            #initialize
            writer = tf.summary.FileWriter("C://Users//Raytine//Anaconda3//envs//TensorBoard//lenet-5-2", sess.graph)
            ...
            for i in range(TRAINING_STEPS):
                ...
                summary,_ = sess.run([merged,train_step],feed_dict={x: reshaped_xs, y_: ys})  
                writer.add_summary(summary,i) 

                #绘图
                if i % 100 == 0:
                    l = sess.run(loss,feed_dict = {x: reshaped_xs, y_: ys})
                    i_c.append(i)
                    loss_c.append(l)
